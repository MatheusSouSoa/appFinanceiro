...
export default function Sumario(props: SumarioProps){
    ...
    return (
        <div className={` grid grid-cols-1 md:grid-cols-3 gap-4`}>
            <SumarioItem titulo="Receitas" valor={receitas} icone={<IconCash/>} iconeClassName="text-green-500"/>
            <SumarioItem titulo="Despesas" valor={despesas} icone={<IconCreditCard/>} iconeClassName="text-red-500"/>
            <SumarioItem titulo="Total" valor={total} icone={<IconArrowsDoubleSwNe/>} iconeClassName="text-yellow-500" valorClassName={total > 0 ? 'text-green-500' : total < 0 ? 'text-red-500' : ''} />
        </div>
    )
}